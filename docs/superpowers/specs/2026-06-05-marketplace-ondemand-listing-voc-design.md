...
| **美客多** | ✅ **已做通** | items API 强制 OAuth(403),改走**商品页真浏览器 + 滚动等待**(价格/评论懒加载)。listing 从 **JSON-LD** 解析(价格/标题/评分/评论数/图片),评论从 DOM 解析。**端到端验证通过**(价格 208739 ARS + 评分 4.6 + 评论 → 入库)。⚠️ 住宅 IP 信誉时好时坏,需多次切代理重试(`_MAX_RETRY=6`),生产高频建议偏好已验证区域。 |
| **Shopee** | ❌ **现有工具绕不过反爬,暂不可用** | 实测(住宅代理+真浏览器+滚动,SG/ID):商品页/搜索页被弹到 `/verify/traffic/error?type=4`(反爬流量墙),内部 API 403 `action_type:2`。**卡在流量墙而非登录墙**——bot 流量在访问任何商品前先被拦,Lazada/美客多能过的"住宅代理+真浏览器"组合在 Shopee 失效(强一个数量级)。绕过需:登录 cookie 注入 / 付费第三方 API(Apify 等) / 官方 partner API / 更强反检测浏览器——均需额外资源,待选型。 |
