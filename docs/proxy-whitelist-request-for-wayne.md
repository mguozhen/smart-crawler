...
请把 smart-crawler NAS 生产机的出口 IP **`99.0.84.158`** 加入美国办公室住宅代理的来源白名单，或提供一个不依赖公网来源白名单的内网/Tailscale 访问入口。
...
`99.0.84.158` 是家宽公网 IP，可能变化。建议优先选下面一种长期方案：

1. **Tailscale/内网访问入口（推荐）**  
   如果办公室代理设备也能加入同一个 Tailscale 网络，给 NAS 一个 Tailscale IP/主机名访问代理，避免公网白名单反复维护。

2. **公网强认证入口**  
   代理端只依赖账号密码/强认证，不再依赖来源 IP 白名单。适合后续更多 worker 或云机器接入。
