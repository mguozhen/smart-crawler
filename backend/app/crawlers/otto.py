...
批C 收编（2026-06）：
  - discovery curl 段改用 make_fetcher().get()，自动计 api_calls
  - warmup + PDP stealth 段用 count_browser_fetch 包裹，成功计 browser_opens
  - 删 proxy 自管(_session)；保留 _headers() + guard / snapshot / sleep / 解析
  - Kasada warmup / persistent profile / KPSDK cookie 写入逻辑全部原样保留
"""
...
class OttoCrawler(BaseCrawler):
    ...
    def crawl(self) -> CrawlResult:
        ...
        urls = self._discover_urls(result)
        if not urls:
    ...
    # ------------------------------------------------------------------
    def _discover_urls(self, result: CrawlResult) -> list[str]:
        fetcher = self.make_fetcher(kind="listing", source="otto")
        seen: set[str] = set()
        ordered: list[str] = []
        for path in _SEED_PATHS:
            seed_url = self.base + path
            try:
                res = fetcher.get(seed_url, headers=self._headers(), timeout=30)
                self.guard(res.status or 0, f"seed:{path}")
            except BlockedError:
                raise
            except Exception as exc:
            ...
            if (res.status or 0) != 200:
                result.notes.append(f"  · seed {path} {res.status}")
                continue

            new = 0
            for href in _PROD_HREF_RE.findall(res.text):
                url = self.base + href
    ...
    def _warm_profile(self, kw: dict) -> bool:
        ...
        try:
            ...
            return False
        try:
            warmup_url = self.base + "/"

            def _do_fetch():
                return StealthyFetcher.fetch(warmup_url, **kw)
    ...
    def _fetch_pdp(self, url: str, kw: dict) -> str | None:
        ...
        批C：StealthyFetcher.fetch 调用用 count_browser_fetch 包裹，
        成功时自动 browser_opens += 1。Kasada profile / cookie 复用逻辑
        全部原样保留（kw 参数含 persist_profile_key，profile 目录不变）。
        """
        try:
            from scrapling.fetchers import StealthyFetcher
        except Exception:
        ...
        try:
            ...
            def _success(page) -> bool:
                ...
                # 挑战 stub 通常 <1KB，正常 PDP 800KB+；阈值 50KB
